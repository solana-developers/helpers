...
export class BankrunConnection {
	...
	async sendTransaction(
		...
		while (finalizedCount < 10) {
			const signatureStatusResponse = await this.getSignatureStatus(signature);
			const signatureStatus = signatureStatusResponse.value?.confirmationStatus;
			if (!signatureStatus) {
				throw new Error('Signature status is null');
			}
	...
	async getTransaction(
		...
		if (txMeta === undefined || txMeta.meta === null) {
			return null;
		}
		const transactionStatus = await this._banksClient.getTransactionStatus(
			signature
		...
		const meta: BankrunTransactionMetaNormalized = {
			...
			err: txMeta.result as TransactionError,
		};
		return {
			slot: Number(transactionStatus?.slot),
			meta,
	...
	async simulateTransaction(
		...
		const returnDataProgramId = simulationResult.meta?.returnData?.programId.toBase58() ?? '';
		const returnDataNormalized = simulationResult.meta?.returnData?.data
